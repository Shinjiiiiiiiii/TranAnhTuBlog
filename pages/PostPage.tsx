...
const PostPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (foundPost) {
      ...
      document.title = "Không tìm thấy trang | Trần Anh Tú";
    }
  }, [id]);
  ...
  const headings = post.blocks
    .map((block, index) => (block.type === "heading" ? { content: block.content as string, index } : null))
    .filter((h) => h !== null) as { content: string; index: number }[];
            ...
            <div className="sticky top-24">
              ...
              <nav className="space-y-2">
                {headings.map((heading) => (
                  <a
                    key={heading.index}
                    href={`#heading-${heading.index}`}
                    className="block text-sm text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors py-1.5 px-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
                  >
                    {heading.content}
                  </a>
                ))}
